...
/*   7线接口SPI-OLED
 * 描述: 本驱动依赖hal库的硬件spi，0模式，不开启DMA,cs默认接地(此驱动有写出来),芯片为SSD1306
 * 功能: oled命令以及数据传递
 * 注:开启DMA要将DMA结构体进行extern
 */

/*选择是否使用DMA(注释来选择是否开启)*/
#define		UseDMA		No		/*不使用DMA*/
//#define		UseDMA		YES 	/*使用DMA*/
...
#define		OLED_RESET_GPIO_Port		GPIOC		/*OLED使用的RES片选引脚*/
#define     OLED_RESET_GPIO_PIN			GPIO_PIN_4
...
返回  值：无
**************************************************************************/
void OLED_WriteSRAM(uint8_t X, uint8_t Page, uint8_t Data)
{
	OLED_DisplayBuf[Page][X]=Data;
}
...
/**************************************************************************
函数功能：OLED指定区域清零
入口参数： X 指定区域左上角的横坐标，范围：-32768~32767，屏幕区域：0~127
		Y 指定区域左上角的纵坐标，范围：-32768~32767，屏幕区域：0~63
